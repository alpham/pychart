...
class T(basecanvas.T):
    ...
    def moveto(self, x, y):             #
        ...
        d = ' '.join([self.__cur_element.getAttribute(
            'd'), 'M', repr(x), repr(-y)]).strip()
    ...
    def text_show(self, font_name, size, color, string):
        ...
        if modifiers:
            if re.search('Italic', modifiers):
                ...
                font_style = 'oblique'
            if re.search('Bold|Demi', modifiers):
                font_weight = 'bold'
            if re.search('Narrow', modifiers):
        ...
        self.__cur_element.appendChild(
            self.__doc.createTextNode(string.encode('utf-8')))
    ...
    def verbatim(self, string):
        self.__cur_element.appendChild(self.__doc.createComment(
            'verbatim not implemented: ' + string))
