...
class T(chart_object.T):
    ...
    def __init__(self, colour=True, **args):
        ...
        if colour:
            ...
            # besides, I want the first two colors to resemble those of
            # gnuplot's postscript terminal
    ...
    def check_integrity(self):
        ...
        if (360 % nSectors != 0):
            raise Exception('Length of dataset ' +
                            str(nSectors) + ' not a divisor of 360 degrees!')
    ...
    def draw(self, ar, can):
        ...
        # each following sector diagram will have its sector width decremented
        # by half this amount (in degrees)
        sector_decrement = 1. / (len(self.data) * 2) * self.sector_width
        i = 0
        for dataset in self.data:
            ...
            if not i:  # draw directions around sector diagram once off
                dir_offset = base_radius + \
                    (self.dir_offset or base_radius * 0.04)
                directions = ['N', 'E', 'S', 'W']
                ...
                for d in directions:
                    # coords for bottom left corner of box
                    x_label, y_label = pychart_util.rotate(
                        dir_offset, 0, angle)
                    tw = font.text_width(d)
                    half = 1 / \
                        3.  # normal arithmetic does not seem to apply to these text_box objects...
                    if (angle == 0):  # east
                        # move down half
                        y_label -= font.text_height(d)[0] * half
                    elif (angle == -180):  # west
                        # move down half
                        y_label -= font.text_height(d)[0] * half
                        ...
                        # move left half
                        x_label -= font.text_height(d)[0] * half
                    elif (angle == -90):  # south
                        # move down (couldn't figure out how to set this
                        # dynamically so I fudged...)
                        y_label -= font.text_height(d)[0] * .8
                        # move left half
                        x_label -= font.text_height(d)[0] * half
            ...
            for val in dataset[self.data_col]:  # now draw the sectors
                ...
                # these may seem confusing, but remember that we need to go
                # counterclockwise
                stop = cur_angle - i * sector_decrement

                can.ellipsis(self.line_style, fill,
                             x_center, y_center, radius, 1, start, stop, self.shadow)
                # we want to go in anticlockwise direction (North, West, South,
                # etc. as in meteorology)
                cur_angle = (cur_angle - self.sector_width) % 360
            i = (i + 1) % len(self.fill_styles)
