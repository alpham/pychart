...
import re
from . import theme
from .afm import dir as afm_dir
...
def _intern_afm(font, text):
    ...
    r = afm_dir.afm.get(font, None)
    if r:
        return r

    font2 = _font_aliases.get(font, None)
    if font2:
        r = afm_dir.afm.get(font2, None)
        if r:
            return r

    try:
        exec("import pychart.afm.%s" % re.sub("-", "_", font))
        return afm_dir.afm[font]
    except:
    ...
    if font2:
        try:
            ...
            return afm_dir.afm[font2]
        except:
            if font not in _undefined_font_warned:
                pychart_util.warn(
                    'Warning: unknown font "%s" while parsing "%s"' % (font, text))
                _undefined_font_warned[font] = 1
    return None
