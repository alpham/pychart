...
def _convert_item(v, typ, line):
    if typ == "a":
        ...
        raise ValueError(
            "Unknown conversion type, type=%s; line=%s" % (typ, line))
...
"""
    ...
    try:
        ...
        raise IndexError("bad data: %s,xcol=%d,ycol=%d,width=%d" %
...
def fread_csv(fd, delim=','):
    ...
    pychart_util.warn(
        "chart_data.fread_csv is deprecated. Use read_csv instead.")
...
"""
    ...
    try:
        ...
        raise IndexError("bad data: %s,xcol=%d,ycollist=%s" %
                         (data, xcol, ycollist))
...
"""
    ...
    try:
        for elem in data:
            ...
            out.append((elem[xcol], mean, stddev,
