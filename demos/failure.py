...
ar.add_plot(line_plot.T(label="no failure", data=data,
                        ycol=4, line_style=line_style.gray50))
ar.add_plot(line_plot.T(label="one failure", data=data, ycol=1))
ar.add_plot(line_plot.T(label="three failures", data=data,
                        ycol=2, line_style=line_style.black_dash2))
...
def describeEvent(seconds, label, off):
    ...
    tb = text_box.T(text=label, loc=(x1 + off, yloc),
                    shadow=(2, -2, fill_style.gray70))
